...
class PhysicalDiskSerializer(serializers.ModelSerializer):
    physicaldisk=serializers.SerializerMethodField()
    # 列表表头verbose_name
    # headers=serializers.SerializerMethodField()
    # 中文名称显示
    ...
    class Meta:
        ...
        # fields='__all__'
        fields=('id','disk_name','disk_space','disk_free_apace')
        # depth=1

    def get_physicaldisk(self,row):
        # print(row._meta.get_field('cpu').verbose_name)
        # 外键的反向查找
        host_obj_list=row.disk_mount_id_set.all()
        ret=[]
        for obj in host_obj_list:
            ret.append({'hostname':obj.machine_name,})
        return ret
